<template>
  <main class="flex flex-col min-h-screen bg-blue-100">
    ...
    <section
      v-if="!studentFilter.sortBy.active"
    ...
    >
      ...
      <div
        v-if="studentFilter.studentsIndex.current.length < 74"
        class="w-full"
      >
        <p class="my-4 text-center">
          <button class="py-2 px-4 rounded bg-white" @click="getAllStudents">
            See all
          </button>
        </p>
      </div>
    ...
    >
      <div
        v-show="studentFilter.sortBy.rarity[3].length > 0"
        class="flex flex-row"
      >
        <div
          class="flex-none flex flex-col justify-center items-center w-12 h-full p-1 rounded bg-yellow-300"
        >
      ...
      <div
        v-show="studentFilter.sortBy.rarity[2].length > 0"
        class="flex flex-row"
      >
        <div
          class="flex-none flex flex-col justify-center items-center w-12 h-full p-1 rounded bg-yellow-300"
      ...
      <div
        v-show="studentFilter.sortBy.rarity[1].length > 0"
        class="flex flex-row"
      >
        <div
          class="flex-none flex flex-col justify-center items-center w-12 h-full p-1 rounded bg-yellow-300"
...
export default {
  components: {
    SearchSettings,
    CardViewComplete: defineAsyncComponent(() =>
      import("@/components/CardViewComplete.vue")
    ),
    CardViewBasicInfo: defineAsyncComponent(() =>
      import("@/components/CardViewBasicInfo.vue")
    ),
    CardViewOnlyChar: defineAsyncComponent(() =>
      import("@/components/CardViewOnlyChar.vue")
    ),
  },
  setup() {
    ...
    function getAllStudents() {
      studentFilter.studentsIndex.current = [
        ...Array(studentFilter.studentsIndex.all).keys(),
      ];
    }
